...
draw_text(bbox_left + 100, bbox_top + textborder, "amount");

itemleftstart = bbox_left + -40
itemtopstart = bbox_top + 30

for (i = 0; i < inventoryendat; ++i){
	for(j = 0; j < playerinventorywidth; ++j){
		if(j == 1)
		draw_text(itemleftstart + 140, itemtopstart + (i* 16), ds_grid_get(myitems, 1, i));
		if(j == 3)
			draw_sprite(ds_grid_get(myitems, j, i), 0, bbox_left + 11, itemtopstart + (i * 16));
